...
def determine_tiers():
    ...
    connect = sqlite3.connect('olympic_stats.db')
    connect.execute('DROP TABLE IF EXISTS olympic_stats')
    connect.execute('''
        CREATE TABLE olympic_stats_test (
    ...
    for index, row in previous_scores.iterrows():
        ...
        connect.execute('''
            INSERT INTO olympic_stats_test
            VALUES (?, ?, ?, ?);
            ''',
            (row.country, beijing, london, rio)
        )
        connect.commit()
    
    abc = connect.execute("SELECT * from olympic_stats_test")
    print(abc)
    connect.close()
...
def read_file(file: str, column_name: str, table):
    with open(file, newline = '') as medal_table:
        table[column_name] = math.nan
        ...
        for row in reader:
            ...
            if (country in table['country'].unique()):
                ...
                table.at[country_idx, column_name] = score
            else:
                table.loc[len(table)] = {'country': country, column_name: score}
